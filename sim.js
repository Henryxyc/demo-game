...
(function (root) {
  ...
  function createEngine(theme) {
    ...
    function forcedChance(combat, g) {
      ...
      var base;
      if (combat >= 1000000) base = 0.40;            /* 100w+ 战力：封顶 40% */
      else if (combat < 50000) base = 0.05;          /* <5w：5% */
      else if (combat < 200000) {                   /* 5w→20w：5% → 18% 线性 */
        var t = (combat - 50000) / 150000;
        base = 0.05 + t * (0.18 - 0.05);
      } else if (combat < 1000000) {                /* 20w→100w：18% → 30% 线性 */
        var t2 = (combat - 200000) / 800000;
        base = 0.18 + t2 * (0.30 - 0.18);
      } else {                                       /* 100w→1000w：30% → 40% 线性 */
        var t3 = (combat - 1000000) / 9000000;
        base = 0.30 + t3 * (0.40 - 0.30);
      }
